...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.startState = (self.startingPosition, tuple(corners_visited))
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append(((next_position, tuple(corners_visited)), action, step_cost))
...
def cornersHeuristic(state, problem):
    ...
    corners_unvisited = list(state[1])
    unvisited_corners_xy = []

    for i in range(3):
        if corners_unvisited[i] == False:
            unvisited_corners_xy.append(corners[i])

    def findClosestAndFarthest(startingPoint, state):
        closestPoint = (0, 0)
        closestPointDist = 999
        farthestPoint = (0, 0)
        farthestPointDist = 0

        for i in state:
            dist = mazeDistance(startingPoint, i, problem.startingGameState)
            if dist < closestPointDist:
                closestPoint = i
                closestPointDist = dist

            if dist > farthestPointDist:
                farthestPoint  = i
                farthestPointDist = dist

        return [closestPoint, closestPointDist, farthestPoint, farthestPointDist]
    
    arr = findClosestAndFarthest(current_position, unvisited_corners_xy)
    
    # closestToFarthestDist = util.manhattanDistance(arr[0], arr[2])

    heuristic = arr[3]


    # def findClosestCornerFood(starting_point, state):
    #     global heuristic
    #     closestPoint = (0, 0)
    #     closestPointDist = 999
    ...
    #     for i in state:
    #         dist = util.manhattanDistance(starting_point, i)

    #         if dist < closestPointDist:
    #             closestPoint = i
    #             closestPointDist = dist
         
    #     heuristic = heuristic + closestPointDist
    #     new_state = state.remove(closestPoint)

    #     return findClosestCornerFood(closestPoint, new_state)

    # return findClosestCornerFood(current_position, unvisited_corners_xy)

    # #calculate the distance from current node to all corner nodes
    # if len(corners_unvisited) > 0:
...
def foodHeuristic(state, problem):
    ...
    food_uneaten_xy = foodGrid.asList()
    ...
    arr = findClosestAndFarthest(current_position, food_uneaten_xy)
